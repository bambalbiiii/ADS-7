...
void drawLine(std::vector<unsigned char>& img, int x0, int y0, int x1, int y1, Color c) {
...
void drawChar(std::vector<unsigned char>& img, int x, int y, char ch, Color c) {
...
void drawText(std::vector<unsigned char>& img, int x, int y, const std::string& s, Color c) {
    for (char ch:s){drawChar(img,x,y,ch,c);x+=7;}
}
...
int main() {
    ...
    for (int n = 2; n <= 100; n++) {
        ...
        { Train t; for(int i=0;i<n;i++) t.addCar(false); t.getLength(); opFalse=t.getOpCount(); }
        { Train t; for(int i=0;i<n;i++) t.addCar(true);  t.getLength(); opTrue=t.getOpCount(); }
        { Train t; for(int i=0;i<n;i++) t.addCar(std::rand()%2); t.getLength(); opRandom=t.getOpCount(); }
        ns.push_back(n);
    ...
    auto toX = [&](int n){ return ML + (n-xmin)*pw/(xmax-xmin); };
    auto toY = [&](int v){ return MT+ph - v*ph/vmax; };

    for (int i=1;i<(int)ns.size();i++) {
        drawLine(img, toX(ns[i-1]), toY(opFalseV[i-1]), toX(ns[i]), toY(opFalseV[i]), {220,50,50});
        drawLine(img, toX(ns[i-1]), toY(opTrueV[i-1]),  toX(ns[i]), toY(opTrueV[i]),  {50,150,50});
        drawLine(img, toX(ns[i-1]), toY(opRandV[i-1]),  toX(ns[i]), toY(opRandV[i]),  {50,50,220});
    }

    for (int i=0;i<(int)ns.size();i+=20) {
        int x=toX(ns[i]);
